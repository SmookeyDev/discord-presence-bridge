...
function getPresence(info) {
	function getMS(string) {
		...
		const seconds =
			string.split(':').length - 1 > 1 ? +a[0] * 3600 + +a[1] * 60 + +a[2] : +a[0] * 60 + +a[1];
		return seconds * 1000;
	}
	try {
		if (document.getElementsByClassName('playControl')[0].classList.contains('playing')) {
			return {
				...
				presence: {
					state: document.getElementsByClassName('playbackSoundBadge__titleContextContainer')[0]
						.firstElementChild.textContent,
					details: document.getElementsByClassName('playbackSoundBadge__titleLink')[0]
						.lastElementChild.textContent,
					startTimestamp:
						Date.now() -
						getMS(
							document.getElementsByClassName('playbackTimeline__timePassed')[0].lastElementChild
								.textContent,
						),
